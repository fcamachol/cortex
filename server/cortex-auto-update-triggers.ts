...
export async function createCortexAutoUpdateTriggers() {
  ...
  try {
    ...
    await db.execute(sql`CREATE TRIGGER trg_persons_updated_at BEFORE UPDATE ON cortex_entities.persons FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_companies_updated_at BEFORE UPDATE ON cortex_entities.companies FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_groups_updated_at BEFORE UPDATE ON cortex_entities.groups FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_objects_updated_at BEFORE UPDATE ON cortex_entities.objects FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    ...
    await db.execute(sql`CREATE TRIGGER trg_projects_updated_at BEFORE UPDATE ON cortex_projects.projects FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_tasks_updated_at BEFORE UPDATE ON cortex_projects.tasks FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_milestones_updated_at BEFORE UPDATE ON cortex_projects.milestones FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_time_entries_updated_at BEFORE UPDATE ON cortex_projects.time_entries FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_resources_updated_at BEFORE UPDATE ON cortex_projects.resources FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_comments_updated_at BEFORE UPDATE ON cortex_projects.comments FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    ...
    await db.execute(sql`CREATE TRIGGER trg_accounts_updated_at BEFORE UPDATE ON cortex_finance.accounts FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_transactions_updated_at BEFORE UPDATE ON cortex_finance.transactions FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_bills_payable_updated_at BEFORE UPDATE ON cortex_finance.bills_payable FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_bills_receivable_updated_at BEFORE UPDATE ON cortex_finance.bills_receivable FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_categories_updated_at BEFORE UPDATE ON cortex_finance.categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
    await db.execute(sql`CREATE TRIGGER trg_budget_categories_updated_at BEFORE UPDATE ON cortex_finance.budget_categories FOR EACH ROW EXECUTE FUNCTION update_updated_at_column()`);
