...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  // WhatsApp Instance Linking Routes
  app.get('/api/whatsapp/instances/available/:userId', async (req: Request, res: Response) => {
    try {
      const { userId } = req.params;
      
      // Get instances where user is owner OR instances marked as shared
      const instances = await storage.getAvailableWhatsappInstances(userId);
      res.json(instances);
    } catch (error) {
      console.error('Error fetching available instances:', error);
      res.status(500).json({ error: 'Failed to fetch available instances' });
    }
  });

  app.post('/api/crm/contacts/link-whatsapp-instance', async (req: Request, res: Response) => {
    try {
      const { instanceName, contactId } = req.body;
      
      if (!instanceName || !contactId) {
        return res.status(400).json({ error: 'Instance name and contact ID are required' });
      }

      // Get instance details including owner JID (phone number)
      const instance = await storage.getWhatsappInstanceByName(instanceName);
      if (!instance) {
        return res.status(404).json({ error: 'WhatsApp instance not found' });
      }

      // Extract phone number from owner JID (remove @s.whatsapp.net suffix)
      const phoneNumber = instance.ownerJid?.replace('@s.whatsapp.net', '') || '';
      
      if (!phoneNumber) {
        return res.status(400).json({ error: 'Instance has no associated phone number' });
      }

      // Create phone number entry for the contact
      const phoneData = {
      ...
      const createdPhone = await storage.createContactPhone(phoneData);
      
      // Update contact's WhatsApp linking
      await storage.updateContactWhatsAppLink(contactId, {
        whatsappJid: instance.ownerJid,
      ...
      console.error('Error linking WhatsApp instance:', error);
      res.status(500).json({ error: 'Failed to link WhatsApp instance' });
    }
  });
