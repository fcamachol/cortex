...
## Changelog
- June 27, 2025: AUTOMATIC MAIN CONTACT CREATION WITH WHATSAPP LINKING COMPLETED - Implemented system where each user account automatically gets one main contact record that serves as the central hub for CRM relationships, main contact is automatically created on first access to contacts system, automatically detects user's WhatsApp instance owner JID and links phone numbers to proper WhatsApp instance and contact data, enhanced CRM contact phones schema with whatsapp_jid and whatsapp_instance_name fields for proper WhatsApp integration, main contact creation now extracts phone number from owner JID (e.g., 5215585333840@s.whatsapp.net → 5215585333840) and creates phone record with hasWhatsApp=true and proper instance mapping
